...
export function AppointmentDetailModal({
  ...
  const createInvoiceMutation = useMutation({
    mutationFn: async () => {
      ...
      console.log(
        ...
        'tipi:',
        typeof invoiceData.discountRate,
      );
      ...
      try {
        ...
        console.error('Fatura oluşturma API hatası:', apiError.response?.data || apiError.message);
        throw error;
      }
    },
    onSuccess: () => {
    ...
    onError: (error: unknown) => {
      ...
      const errorMessage =
        apiError.response?.data?.message || 'Fatura oluşturulurken bir hata oluştu';
      toast({
        title: 'Hata',
        description: Array.isArray(errorMessage) ? errorMessage.join(', ') : errorMessage,
        variant: 'destructive',
      });
      console.error('Fatura oluşturma hatası detayları:', apiError.response?.data);
    },
  });
  ...
  const remainingSessions =
    isPackageAppointment && appointment.customerPackage?.remainingSessions
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{formattedDate} tarihli randevu bilgileri</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <div className="text-sm">
                <p className="font-medium">{appointment.customer?.name || 'İsimsiz Müşteri'}</p>
                ...
                <Card className="border border-green-200 bg-green-50">
                  <CardContent className="p-4 space-y-2">
                    ...
                    <div className="flex items-center text-sm text-green-700 mt-1">
                      ...
                      <span>
                        {appointment.duration || appointment.service?.duration || '?'} dakika
                      </span>
                    </div>
                  </CardContent>
                </Card>
